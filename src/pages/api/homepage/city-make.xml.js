...
const EXTERNAL_DATA_URL = 'https://api.okmechanic.store/customer/sitemap/city';
       ...
       <sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
         ${data
           .map((item) => {
             return `
           <sitemap>
             <loc>${`${process.env.NEXT_PUBLIC_SITE_URL}${item.slug.replaceAll('-','_')}-make-sitemap.xml`}</loc>
           </sitemap>
         `;
           })
           .join('')}
