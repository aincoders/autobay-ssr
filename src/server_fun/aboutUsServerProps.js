...
import { ABOUT_API, CITY_API, CUSTOMER_API, SCHEMA_API, SEO_PAGE_TYPE } from "src/utils/constant";


export default async function aboutUsServerProps(context, city = '', vehicle = { make: '', model: '' }) {
    ...
    async function GetSeoInformation() {
        const params = { page_type: SEO_PAGE_TYPE.aboutUs };
    ...
    async function GetAboutList() {
        return await axios.get(ABOUT_API,'');
    }

    try {
        const [resSeoInfo, resSchema, resAboutList] = await Promise.all([GetSeoInformation(), GetPageSchema(),GetAboutList()]);
        const aboutData = resAboutList.data.result.list;
        return {
            seoInfo: resSeoInfo?.data?.result,
            schemaInfo: resSchema?.data?.result,
            aboutUsInfo:resAboutList.data.result,
            benefitList:aboutData.benefit,
            howItWorkList:aboutData.how_it_works,
            whyChooseList:aboutData.why_choose_us,
            featureList:aboutData.features_list,
            currentCity,
            currentVehicle
        }
    }
