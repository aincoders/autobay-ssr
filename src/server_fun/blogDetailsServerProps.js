...
import { BLOG_API, CITY_API, CUSTOMER_API, SCHEMA_API, SEO_PAGE_TYPE } from "src/utils/constant";


export default async function blogDetailsServerProps(context, city = '', vehicle = { make: '', model: '' }) {
    ...
    async function GetSeoInformation() {
        const params = { page_type: SEO_PAGE_TYPE.blogDetails };
    ...
    async function GetBlogDetails() {
        const params = { slug: query.blogSlug };
        return await axios.get(BLOG_API.details, { params });
    }

    try {
        const [resSeoInfo, resSchema, resBlogDetails] = await Promise.all([GetSeoInformation(), GetPageSchema(),GetBlogDetails()]);
        return {
            seoInfo: resSeoInfo?.data?.result,
            schemaInfo: resSchema?.data?.result,
            blogDetails:resBlogDetails.data.result,
            currentCity,
            currentVehicle
        }
    }
