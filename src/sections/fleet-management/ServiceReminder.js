...
import { AddServiceReminderModal, FleetVehicleModal, SellVehicleModal, VehicleSellConfirmationDialog } from 'src/master';
...
import ServiceReminderTableRow from './list/ServiceReminderTableRow';
...
export default function ServiceReminder() {
    ...
    async function GetList() {
        ...
        const response = await getApiData(CUSTOMER_API.servicereminder, params, signal);
        if (response) {
    ...
    const handleDelete = async (row) => {
        await DeleteItem(row.fleet_vehicle_service_reminder_id)
    };

    async function DeleteItem(fleet_vehicle_service_reminder_id) {
        try {
            const data = {
                fleet_vehicle_service_reminder_id: fleet_vehicle_service_reminder_id,
                customer_vehicle_id: customer_vehicle_id,
            };
            const response = await postApiData(CUSTOMER_API.serviceReminderDelete, data);
            GetList()
        } catch (err) {
            console.log(err)
        }
    ...
    const TABLE_HEAD = [
        { id: 'service_task', label: t('service_task'), isSorting: false },
        { id: 'service_package', label: t('service_package'), isSorting: false },
        { id: 'date', label: t('created_date'), isSorting: false },
        { id: 'next_due', label: t('next_due'), isSorting: false },
                ...
                <Box sx={{ py: { xs: SPACING.xs, md: SPACING.md, } }}>
                    <Grid container rowSpacing={3}>
                        ...
                        <Grid item xs={12} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 2, py: 1, }}>
                            ...
                            {isDesktop && <VehicleTabMenu current={'service_reminder'} responseList={responseList} />}
                            <Box sx={{ borderTop: '1px solid', borderColor: 'divider', bgcolor: 'background.paper' }}>
                                <Box p={1.5} px={3} display="flex" alignItems={'center'} justifyContent="space-between" gap={2}>
                                    <Typography variant="h6" sx={{ width: 200 }}>{t('service_reminder')}</Typography>
                                    <Box display={'flex'} alignItems='center' gap={1} flex={1} justifyContent='flex-end'>
                                        <Button variant="soft" startIcon={<Add />} onClick={() => setServiceReminderModal({ status: true, data: "", customer_vehicle_id: router.query.vehicle })}>
                                            {t('add_service_reminder')}
                                        </Button>
                                    </Box>
                                </Box>
                            </Box>
                                                ...
                                                {loading ?
                                                    ...
                                                    responseList?.list?.length > 0
                                                        ...
                                                        responseList?.list?.map((row, i) => (
                                                            ...
                                                            <ServiceReminderTableRow
                                                                key={i}
                                                                row={row}
                                                                onViewRow={() => console.log("")}
                                                                onEditRow={() => setServiceReminderModal({ status: true, customer_vehicle_id: router.query.vehicle, data: row })}
                                                                onUpdateStatus={() => console.log(row)}
                                                                onDeleteRow={() => handleDelete(row)}

                                                            // setCustomerModal({ status: true, data: row })
                                                            />
                                                        ))
