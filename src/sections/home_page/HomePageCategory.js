...
export default function HomePageCategory() {
    const { loading, categoryList, currentCity, currentVehicle } = useContext(HomePageContext);
    const { make, model } = currentVehicle

    const isDesktop = useResponsive('up', 'lg');


    if (!loading && categoryList.sub_list.length == 0) {
        return null;
    }
            ...
            <Box display={'flex'} flexDirection="column" gap={3}>
                <Box display={'flex'} flexDirection="column" sx={{ textAlign: 'left' }}>
                    <Typography variant={isDesktop ? 'h3' : 'h6'}>
                        {replaceString(categoryList.homepage_section_title)}
                    </Typography>
                    <Typography variant={isDesktop ? 'body1' : 'caption'} color={'text.secondary'}>
                        {replaceString(categoryList.homepage_section_description)}
                    </Typography>
                </Box>
                <Grid container spacing={{ xs: 2, md: 4 }}>
                    {(loading ? Array.from(new Array(12)) : categoryList.sub_list).map(
                        (response, index) => {
                            ...
                            return (
                                <Grid item xs={3} sm={3} md={1.71} key={index}>
                                    {response ? (
                                        <Box
                                            display={'flex'}
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                >
                                                    <Typography
                                                        variant={isDesktop ? 'button' : 'caption'}
                                                        sx={{
                                                            color: 'white',
                                                            position: 'absolute',
                                                            right: 0,
                                            ...
                                            <Typography
                                                ...
                                                sx={{ WebkitLineClamp: 2, overflow: 'hidden', display: '-webkit-box', WebkitBoxOrient: 'vertical', fontWeight: 'medium'}}
                                                textAlign={'center'}
                                            >
