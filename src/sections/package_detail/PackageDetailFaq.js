...
export default function PackageDetailFaq() {
    const { loading, packageDetails, faqList } = useContext(PackageDetailContext);
            ...
            <Box sx={{ py: { xs: SPACING.xs, md: SPACING.md }, }}>
                <Box display={'flex'} flexDirection="column" gap={3}>
                    <Box display={'flex'} flexDirection="column" sx={{ gap: 0.5 }}>
                        <Typography variant={isDesktop ? 'h4' : 'subtitle2'}>{`Frequently Asked Questions (FAQs) on ${packageDetails.service_group_name}`}</Typography>
                    </Box>
